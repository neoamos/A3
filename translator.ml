...
and ast_ize_SL (sl:parse_tree) : ast_sl =
  match sl with
  | PT_nt ("SL", [PT_nt ("S", s); PT_nt ("SL", slist) ]) -> (ast_ize_S (PT_nt ("S", s)) ):: (ast_ize_SL (PT_nt ("SL", slist)))
  | PT_nt ("SL", []) -> []
...
and ast_ize_S (s:parse_tree) : ast_s =
  ...
  | PT_nt ("S", [PT_term "read" ; PT_id id])
    -> AST_read id
  | PT_nt ("S", [PT_term "write"; PT_nt ("E", sl)])
    -> AST_write (ast_ize_expr (PT_nt ("E", sl)))
  |PT_nt ("S", [PT_term "if"; PT_nt ("R", r); PT_nt ("SL", sl); PT_term "fi"])
    -> AST_if (ast_ize_expr (PT_nt ("R", r)), ast_ize_SL (PT_nt ("SL", sl)) )
  | PT_nt ("S", [PT_term "do"; PT_nt ("SL", sl); PT_term "od"])
    -> AST_do  (ast_ize_SL (PT_nt ("SL", sl)))
  | PT_nt ("S", [PT_term "check"; PT_nt ("R", sl)])
    -> AST_check (ast_ize_expr (PT_nt ("R", sl)))
  | _ -> raise (Failure "malformed parse tree in ast_ize_S")

and ast_ize_expr (e:parse_tree) : ast_e =
  (* e is an R, E, T, or F parse tree node *)
  match e with
  | PT_nt ("R", [expr; PT_nt ("ET", etl)]) -> ast_ize_reln_tail ( ast_ize_expr expr ) ( PT_nt ("ET", etl) )
  | PT_nt ("E", [expr; PT_nt ("TT", ttl)]) -> ast_ize_expr_tail ( ast_ize_expr expr ) ( PT_nt ("TT", ttl) )
  | PT_nt ("T", [expr; PT_nt ("FT", ftl)]) -> ast_ize_expr_tail ( ast_ize_expr expr ) ( PT_nt ("FT", ftl) )
  | PT_nt ("F", [PT_id id]) -> AST_id id
  | PT_nt ("F", [PT_num num]) -> AST_num num
  | PT_nt ("F", [PT_term "("; PT_nt ("E", el); PT_term ")"]) -> ast_ize_expr (PT_nt ("E", el))
...
and ast_ize_reln_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  ...
  match tail with
  | PT_nt ("ET", [PT_nt ("ro", [PT_term ro]); PT_nt ("E", el)]) -> AST_binop (ro, lhs, (ast_ize_expr (PT_nt ("E", el))))
  | PT_nt ("ET", []) -> lhs
...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  ...
  match tail with
  | PT_nt ("TT", [PT_nt ("ao", [PT_term ao]); PT_nt ("T", tl); PT_nt ("TT", ttl)])
    -> ast_ize_expr_tail ( AST_binop (ao, lhs, ast_ize_expr (PT_nt ("T", tl))) ) (PT_nt ("TT", ttl))
  | PT_nt ("TT", []) -> lhs
  | PT_nt ("FT", [PT_nt ("mo", [PT_term mo]); PT_nt ("F", fl); PT_nt ("FT", ftl)])
    -> ast_ize_expr_tail ( AST_binop (mo, lhs, ast_ize_expr (PT_nt ("F", fl))) ) (PT_nt ("FT", ftl))
  | PT_nt ("FT", []) -> lhs
